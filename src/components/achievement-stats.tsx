import { BadgeCheck, Trophy, BookOpen } from "lucide-react";
import {
  achievementItems as staticAchievementItems,
  categoryMeta,
...
const items: { cat: AchievementCategory; icon: typeof BadgeCheck }[] = [
  { cat: "certification", icon: BadgeCheck },
  { cat: "award", icon: Trophy },
  { cat: "publication", icon: BookOpen },
];

export function AchievementStats({ items: achievementItems = staticAchievementItems }: { items?: AchievementItem[] }) {
  const { language } = useLanguage();
  return (
    <div className="grid grid-cols-3 gap-3 md:gap-4">
      {items.map(({ cat, icon: Icon }) => {
        const count = achievementItems.filter((i) => i.category === cat).length;
